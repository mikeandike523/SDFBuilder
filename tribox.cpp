...
//credit: https://gist.github.com/jflipts/fc68d4eeacfcc04fbdb2bf38e0911850
...
int pointInsideBox(vec3 A, vec3 boxCenter, vec3 boxRadius){
    float minX =boxCenter.x-boxRadius.x; float maxX=boxCenter.x+boxRadius.x;
    float minY =boxCenter.y-boxRadius.y; float maxY=boxCenter.y+boxRadius.y;
    float minZ =boxCenter.z-boxRadius.z; float maxZ=boxCenter.z+boxRadius.z;
    if(A.x<minX||A.x>maxX) return 0;
    if(A.y<minY||A.y>maxY) return 0;
    if(A.z<minZ||A.z>maxZ) return 0;
...
int triboxInside(vec3 A, vec3 B, vec3 C, vec3 boxCenter, vec3 boxRadius){
    if(pointInsideBox(A,boxCenter,boxRadius)) return 1;
    if(pointInsideBox(B,boxCenter,boxRadius)) return 1;
    if(pointInsideBox(C,boxCenter,boxRadius)) return 1;
    return 0;
}
